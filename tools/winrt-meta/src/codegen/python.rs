...
fn reorder_getters_before_setters(methods: &[MethodMeta]) -> Vec<&MethodMeta> {
    ...
    // Emit: getters, then setters (so @property is defined before @x.setter), then others
    let mut result = Vec::with_capacity(methods.len());
    // Pair getters with their matching setters
    for g in &getters {
        result.push(*g);
    ...
    // Any remaining setters without a getter
